...
TEST( Derivative, square_TB_fermi_surface ) {
  ...
  double E_F = (Ekm_min + Ekm_max)/2.0; // TODO scan E_F from min(E) to max(E)
  ...
  for (std::size_t d = 0; d < k_dim; d++) {
    ...
    auto local_rows = std::get<0>(local_d_rho0_dk.at(d));
    ASSERT_EQ( begin, local_rows.at(0) );
    ASSERT_EQ( end, local_rows.at(num_local_rows - 1) + 1 );
  }

  std::vector<PetscInt> local_rows = std::get<0>(local_d_rho0_dk.at(0));
  ...
  auto local_rho0 = anomtrans::get_local_contents(rho0_km);

  json j_out = {
    {"k_comps", local_k_comps},
    {"ms", local_ms},
    {"Ekm", std::get<1>(local_Ekm)},
    {"rho0", std::get<1>(local_rho0)},
    {"norm_d_rho0_dk", norm_vals}
  };
